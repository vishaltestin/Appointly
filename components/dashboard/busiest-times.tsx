...
export function BusiestTimes({ data }: { data: BusiestTimesData }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm font-semibold">Busiest times</CardTitle>
      </CardHeader>
      <CardContent>
        {totalBookings === 0 ? (
          <div className="flex flex-col items-center justify-center gap-1 py-8 text-muted-foreground">
            <p className="text-sm">Not enough booking data yet.</p>
            <p className="text-xs">Patterns will appear after a few bookings.</p>
          </div>
        ) : (
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        style={{
                          height: `${Math.max(intensity * 56, 6)}px`,
                          backgroundColor:
                            intensity > 0
                              ? primaryWithOpacity(Math.max(intensity, 0.2))
                              : CHART_COLORS.muted,
                        }}
